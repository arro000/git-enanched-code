...
/**
 * E2E tests for US-008: Applicazione chunk da colonna sinistra con >> e x.
 *
 * These tests verify that the custom merge editor renders action buttons
 * (>> and x) for HEAD conflict chunks and that the editor opens correctly
...
suite('US-008: Chunk HEAD Application — E2E', () => {
    let tempDir: string;
    let conflictFilePath: string;

    suiteSetup(() => {
        tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'git-enhanced-us008-'));
    });

    setup(() => {
        conflictFilePath = path.join(tempDir, 'chunk-test.ts');
    ...
    test('custom merge editor opens for file with multiple conflicts', async () => {
        ...
        assert.ok(
            tabConEditorCustom,
            'Custom editor should open for file with multiple conflict blocks'
        );
    });
});
